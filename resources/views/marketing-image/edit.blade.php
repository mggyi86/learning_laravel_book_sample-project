...
@section('content')
    ...
    <br>
    <form class="form" role="form" method="POST" action="{{ url('/marketing-image/' . $marketingImage->id) }}" enctype="multipart/form-data">
        <input type="hidden" name="_method" value="patch">
        {{ csrf_field() }}

        <div class="form-group{{ $errors->has('image_weight') ? ' has-error' : '' }}">
            <label class="control-label">Image Weight</label>
            <input type="number" class="form-control" name="image_weight" value="{{ old('image_weight') ? old('image_weight') : $marketingImage->image_weight }}">
        ...
        <div class="form-group{{ $errors->has('is_active') ? ' has-error' : '' }}">
            ...
            <select class="form-control" id="is_active" name="is_active">
                <option value="{{$marketingImage->is_active}}">{{ $marketingImage->is_active == 1 ? 'Yes' :'No' }}</option>
        ...
        <div class="form-group{{ $errors->has('is_featured') ? ' has-error' : '' }}">
            ...
            <select class="form-control" id="is_featured" name="is_featured">
                <option value="{{$marketingImage->is_featured}}">{{ $marketingImage->is_featured == 1 ? 'Yes' :'No' }}</option>
                <option value="1">Yes</option>
                <option value="0">No</option>
            </select>
